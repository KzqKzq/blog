...
/**
 * Handles image upload with client-side compression and Supabase storage
...
export const handleImageUpload = async (
  ...
  if (file.size > MAX_FILE_SIZE * 2) { // Allow larger files since we'll compress
    throw new Error(
      `File size exceeds maximum allowed (${(MAX_FILE_SIZE * 2) / (1024 * 1024)}MB)`
    )
  }
  ...
  const extension = 'webp' // Always use webp after compression
